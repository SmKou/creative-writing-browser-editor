export const sample = `# Lover in the Flesh
...
// localStorage: use for current work and current draft
// .get|set(chapter_<id>, section[])
// .get|set(section_<id>, paragraph[])
// .get|set(paragraph_<id>, sentence[])
// .get|set(sentence_<id>, sentence)

// sessionStorage: use for current work, not current draft
// + features not in use
...
const current = {
	...
	paragraph: "",
	sentence: ""
} // localStorage.get|set("current_[segment]", id)
...
lines.forEach(line => {
	...
	switch (cmd) {
		case "#":
			const work_id = v4()
			const work = create.work(work_id, content.join(" "))
			works.set(work_id, work)
			current.work = work_id

			const draft_id = v4()
			const draft = create.draft(draft_id)
			drafts.set(draft_id, draft)
			works.get(current.work).drafts.push(draft_id)
			works.get(current.work).current = draft_id
			current.draft = draft_id
			break;
		case "##":
			const chapter_id = v4()
			const chapter = create.chapter(chapter_id, content.join(' '))
			chapters.set(chapter_id, chapter)
			drafts.get(current.draft).order.push(chapter_id)
			drafts.get(current.draft).current.chapter = chapter_id
			break;
		case "###":
			const section_id = v4()
			const section = create.section(section_id, content.join(" "))
			sections.set(section_id, section)
			if (!chapters.get(current.chapter).segment
				&& chapters.get(current.chapter).order.length)
			{
				const chapter = chapters.get(current.chapter)
				const pre_section_id = v4()
				const pre_section = create.section(pre_section_id, "")

				const paragraphs = chapter.order
				while (paragraphs.length)
					pre_section.order.push(paragraphs.shift())
				chapter.order.push(pre_section_id)
				chapter.segment = true
			}
			drafts.get(current.draft).current.section = section_id
			chapters.get(current.chapter).order.push(section_id)
		...
		default:
			...
			sens.forEach(sen => {
				...
				const sentence = create.sentence(section_id, sen)
				sentences.set(sentence_id, sentence)
				paragraph.order.push(sentence_id)
			})
			paragraphs.set(paragraph_id, paragraph)
			drafts.get(current.draft).current.paragraph = paragraph_id
			const sentence_id = paragraph.order.at(-1)
			drafts.get(current.draft).current.sentence = sentence_id
			if (chapters.get(current.chapter).segment)
				sections.get(current.section).order.push(paragraph_id)
			else
				chapters.get(current.chapter).order.push(paragraph_id)
			current.paragraph = paragraph_id
			current.sentence = sentence_id
			break;
	}
})
export const load_sample = {
	works,
	drafts,
	chapters,
	sections,
